...
const App: React.FC = () => {
  ...
  const fetchGames = useCallback(async (userId: string) => {
    setIsLoading(true);
    const { data } = await supabase.from('games').select().eq('user_id', userId).order('completion_date', { ascending: false });
  ...
  const handleSaveGame = async (gameData: Partial<Game>) => {
    ...
    if (gameData.id) {
      await supabase.from('games').update(gameData).eq('id', gameData.id);
    } else {
      await supabase.from('games').insert({ ...gameData, user_id: user.id });
    }
    fetchGames(user.id);
  };
  ...
  const handleDeleteGame = async (id: string) => {
    if (confirm('Deseja realmente remover este registro da sua coleção?')) {
      ...
      try {
        ...
        alert("Erro ao remover o jogo. Tente novamente.");
  ...
  return (
    <div className="min-h-screen pb-20 bg-slate-950">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 py-6">
        {/* Google AdSense Area - Adicionado conforme solicitado */}
